...
let EnhancedTableToolbar = props => {
  const { numSelected, classes, onDelete, onEdit } = props;
  ...
  return (
    ...
    >
      ...
      <div className={classes.actions}>
        {numSelected === 1 ? (
          <Fragment>
            <Tooltip title="Edit" className={classes.actionBtn}>
              <IconButton aria-label="Edit" onClick={onEdit}>
                <EditIcon />
              </IconButton>
            </Tooltip>
            <Tooltip title="Delete" className={classes.actionBtn}>
...
class EnhancedTable extends React.Component {
  ...
  handleSelectAllClick = event => {
    if (event.target.checked) {
      this.setState((state, props) => ({
        selected: props.data.map(n => n.id)
      }));
      return;
    }
  ...
  handleDeleteBook = () => {
    ...
    if (selected.length > 1) {
      // TODO: Custom alert message
      console.log(`Can't delete more than one book at once`);
    }
  ...
  handleEditBook = () => {
    ...
    if (selected.length > 1) {
      // TODO: Custom alert message
      console.log(`Can't edit more than one book at once`);
    }
